...
def LeerArchivo():
    ...
    global FondosFI
    global Circular
    global CambioEmpresa
    global CrearParametros
    global ConsultaConceptos
    global NecesitoAyuda
    global EditarInstancias
    global MantenedorTextos
    ...
    CS=Activar["CS"]
    BS=Activar["BS"]
    FondosFM=Activar["FondosFM"]
    FondosFI=Activar["FondosFI"]
    Circular=Activar["Circular"]
    CambioEmpresa=Activar["CambioEmpresa"]
    CrearParametros=Activar["CrearParametros"]
    ConsultaConceptos=Activar["ConsultaConceptos"]
    NecesitoAyuda=Activar["NecesitoAyuda"]
    EditarInstancias=Activar["EditarInstancias"]
    MantenedorTextos=Activar["MantenedorTextos"]
...
def BuscarInstanciaCheck(self, NombreInst,Accion):
    Valor = 1
    while True:
        try:
            # Construir el xpath para la celda en la columna 2 de la fila actual
            ...
            if Xpath == NombreInst:
                if Accion=="Editar":
                    boton_xpath = f"/html/body/div/div/div[2]/div/div[2]/div[1]/div[2]/div/div[1]/table/tbody/tr[{Valor}]/td[6]/div[1]/button "                
                    # Esperar hasta que el botón sea clickeable
                    boton = WebDriverWait(self.driver, 10).until(
                        EC.element_to_be_clickable((By.XPATH, boton_xpath))
                    )
                    ...
                    return Xpath
                if Accion=="Cargar":
                    boton_xpath = f"/html/body/div/div/div[2]/div/div[2]/div[1]/div[2]/div/div[1]/table/tbody/tr[{Valor}]/td[6]/div[2]/button "                
                    # Esperar hasta que el botón sea clickeable
                    boton = WebDriverWait(self.driver, 10).until(
                        EC.element_to_be_clickable((By.XPATH, boton_xpath))
                    )
                    ...
                    return Xpath
                if Accion=="ManTextos":
                    boton_xpath = f"/html/body/div/div/div[2]/div/div[2]/div[1]/div[2]/div/div[1]/table/tbody/tr[{Valor}]/td[6]/div[3]/button "                                   
                    # Esperar hasta que el botón sea clickeable
                    boton = WebDriverWait(self.driver, 10).until(
                        EC.element_to_be_clickable((By.XPATH, boton_xpath))
                    )
                    ...
                    time.sleep(3)
                    return Xpath
                if Accion=="VerInstancia":
                    boton_xpath = f"/html/body/div/div/div[2]/div/div[2]/div[1]/div[2]/div/div[1]/table/tbody/tr[{Valor}]/td[7]/button "                               
                    # Esperar hasta que el botón sea clickeable
                    boton = WebDriverWait(self.driver, 10).until(
                        EC.element_to_be_clickable((By.XPATH, boton_xpath))
                    )
...
def EditarInstancia(self,PalabraNueva):
        try:
            self.driver.find_element(By.XPATH, "//div[3]/div/div/div/span/span/span").click()
            time.sleep(1)
            self.driver.find_element(By.XPATH, "//span[contains(.,\'4to Trimestre\')]").click()
            time.sleep(1)
            self.driver.find_element(By.XPATH, "//body/div/div/div[2]").click()
            time.sleep(1)
            self.driver.find_element(By.ID, "descripcion").send_keys(Keys.BACKSPACE *30)
            time.sleep(1)
            self.driver.find_element(By.ID, "descripcion").send_keys(f"{PalabraNueva}")
            time.sleep(1)
            screenshot(self.Ruta, "Edicion de Instancia", self.driver)
            time.sleep(2)
            self.driver.find_element(By.XPATH, "//span[contains(.,\'Actualizar Instancia\')]").click()
            time.sleep(1)
            screenshot(self.Ruta, "Instancia Editada", self.driver)
            time.sleep(1)
        except Exception as e:
            logINFO(f"Error EditarInstancia: {str(e)}", "ERROR")
            self.fail(f"Error EditarInstancia: {str(e)}")

def AgregarTextos(self,NombrePrueba,Veces):
        try:
            time.sleep(2)
            self.driver.find_element(By.XPATH, "/html/body/div/div/div[2]/div/div[2]/div[1]/div[2]/div/div[1]/table/tbody/tr[1]/td[4]/button").click()
            time.sleep(2)
            x=1
            while x <= int(Veces):
                self.driver.find_element(By.XPATH, f"/html/body/div/div/div[2]/div/div[3]/div[1]/div[2]/div/div[1]/table/tbody/tr[{x}]/td[4]/button").click()
                time.sleep(2)
                self.driver.switch_to.frame(0)
                self.driver.find_element(By.XPATH, "//p").click()
                time.sleep(2)
                self.driver.find_element(By.XPATH, "/html/body/div").send_keys(f"Empresa de Prueba {x}")
                time.sleep(2)
                self.driver.switch_to.default_content()
                time.sleep(2)
                screenshot(self.Ruta, f"Nuevotexto{x}", self.driver)
                self.driver.find_element(By.XPATH, "//span[contains(.,\'Crear texto\')]").click()
                time.sleep(4)
                x=x+1
            self.driver.find_element(By.XPATH, "/html/body/div/div/div[2]/div/nav/ol[2]/li[1]/span[2]").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, "/html/body/div/div/div[2]/div/nav/ol[1]/li/span").click()
            time.sleep(2)
        except Exception as e:
            logINFO(f"Error {NombrePrueba}: {str(e)}", "ERROR")
            self.fail(f"Error {NombrePrueba}: {str(e)}")

def test_CopiaConfiguracionTextos(self,NombrePrueba,Nombre1,Nombre2):
        try:
            self.driver.find_element(By.XPATH, "//span[contains(.,\'Instancias\')]").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, "//span[contains(.,\'Herramientas\')]").click()
            ...
            self.driver.find_element(By.XPATH, "//li[1]/span/span").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, "//form/div/div/div/div/div/span/span").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, f"//li[contains(.,\'{Nombre1}')]").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, "//div[2]/div/div/div/span/span/span").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, f"//span[contains(.,\'{Nombre2}')]").click()
            time.sleep(2)
            screenshot(self.Ruta, "InstanciasArtualizar", self.driver)
            self.driver.find_element(By.XPATH, "//span[contains(.,\'Actualizar\')]").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, "/html/body/div[2]/div[2]/div[2]/div[2]/button[2]").click()
            time.sleep(2)
            self.driver.find_element(By.XPATH, "/html/body/div/div/div[1]/div/div[2]/ul/li[1]/div").click()
            time.sleep(4)
        except Exception as e:
            logINFO(f"Error {NombrePrueba}: {str(e)}", "ERROR")
            self.fail(f"Error {NombrePrueba}: {str(e)}")

def RevisarTextosAntesdeCrear(self,NombrePrueba,nombreInstBuscar,Veces):
        try:
            time.sleep(2)
            xpath_td = "/html/body/div/div/div[2]/div/div[2]/div[1]/div[2]/div/div[1]/table/tbody/tr[1]/td[3]"   
            elemento_td = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located((By.XPATH, xpath_td))
            )            
            # Obtener el texto del elemento
            Xpath = elemento_td.text
            if(Xpath=="3 / 6"): #Si existen Textos Entrara para eliminarlos
                self.driver.find_element(By.XPATH, "/html/body/div/div/div[2]/div/div[2]/div[1]/div[2]/div/div[1]/table/tbody/tr[1]/td[4]/button").click()#Ver conceptos                
                time.sleep(2)
                screenshot(self.Ruta, "ConceptosAntesdeEliminar", self.driver)
                x=1
                while x <= int(Veces):
                    self.driver.find_element(By.XPATH, f"/html/body/div/div/div[2]/div/div[3]/div[1]/div[2]/div/div[1]/table/tbody/tr[{x}]/td[4]/button").click() #Editar Conceptos
                    time.sleep(2)
                    self.driver.switch_to.frame(0)
                    self.driver.find_element(By.XPATH, "//p").click()
                    time.sleep(2)
                    self.driver.find_element(By.XPATH, "/html/body/div").send_keys(Keys.BACKSPACE *30)
                    time.sleep(2)
                    self.driver.switch_to.default_content()
                    time.sleep(2)
                    screenshot(self.Ruta, f"Nuevotexto{x}", self.driver)
                    self.driver.find_element(By.XPATH, "//span[contains(.,\'Actualizar texto\')]").click()
                    time.sleep(4)
                    x=x+1
                self.driver.find_element(By.XPATH, "/html/body/div/div/div[2]/div/nav/ol[2]/li[1]/span[2]").click() #volver a textos de la instancia
                time.sleep(2)
                screenshot(self.Ruta, "ConceptosEliminados", self.driver)
                self.driver.find_element(By.XPATH, "/html/body/div/div/div[2]/div/nav/ol[1]/li/span").click() #Instancias
                time.sleep(2)
            else:
                self.driver.find_element(By.XPATH, "/html/body/div/div/div[2]/div/nav/ol[1]/li/span").click() #Instancias
                time.sleep(2)
