...
def screenshot(data=None):
    # Tomar capturas de pantalla en todas las pruebas, independientemente del resultado
    take_screenshot = True

    global screen_base, screen_img

    screen_base = HTMLReporter.base_path.replace("\\","/")
...
class HTMLReporter(object):
    ...
    def pytest_terminal_summary(self, terminalreporter, exitstatus, config):
        ...
        if _suite_name is not None:
            ...
            relative_path = os.path.join(directorio_actual, 'estela.png')
            live_logs_file = open(path, 'w')
            message = self.renew_template_text(relative_path)
    ...
    def pytest_runtest_makereport(self, item, call):
        global _suite_name, _previous_suite_name, _current_error, _initial_trigger
        outcome = yield
    ...
    def append_test_metrics_row(self):
        ...
        if (self.rerun is not None) and (max_rerun() is not None):
            ...
            if (_pvalue == max_rerun() + 1) or (_test_status == 'PASS'):
                if ((_test_status == 'FAIL') or (_test_status == 'ERROR')or(_test_status == 'PASS')) and (
    ...
    def renew_template_text(self, logo_url):
        ...
        fecha_formateada = fecha_formateada[0].lower() + fecha_formateada[1:]
        ...
        template_text = template_text.replace("__date__", str(fecha_formateada))
