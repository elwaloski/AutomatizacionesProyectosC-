...
namespace netcore6_GX
{
    public class Tests
    {
        ...
        public void NecesitoAyuda()
        {
            //UtilReporte.CrearTestCase("NecesitoAyuda", "GX");
            string NombrePrueba = "NecesitoAyuda";

            try
            {
                FuncionesWebs.NecesitoAyuda(driver, NombrePrueba);

            }
            catch (Exception ex)
            {
                Assert.Fail($"Error al Cargar Taxonomia: {ex.Message}");
            }
        }
